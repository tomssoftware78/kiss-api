...
class PersoonLikeThisQueryBuilder:
    ...
    def __build_where_for_voornaam(self, voornaam, naam):
        result = ''
        if voornaam:
    ...
    def __build_where_for_naam(self, voornaam, naam):
        result = ''
        if naam:
            if voornaam:
                result = " and "
    ...
    def build_query(self, voornaam, naam):
        ...
        where_clause = "where " 
        where_clause = where_clause + self.__build_where_for_voornaam(voornaam=voornaam, naam=naam)
        where_clause = where_clause + self.__build_where_for_naam(voornaam=voornaam, naam=naam)
